...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      setCurrentUser(user);
      setIsLoggedIn(true);
      if (!activeMenu) setActiveMenu(getDefaultMenu(user.role));
    }
  }, []);
  ...
  const handleLogin = (user) => {
    setCurrentUser(user);
    setIsLoggedIn(true);
    const defaultMenu = getDefaultMenu(user.role);
  ...
  const renderContent = () => {
    ...
    if (isManager) {
      switch (activeMenu) {
                    ...
                    <div className="dp-wrap">
                      ...
                      <div className="dp-bar-section">
                        <div className="dp-bar-track">
                          ...
                          <div className={`dp-bar-fill ${pplHealth.healthRatio >= 3 ? 'dp-fill-ok' : 'dp-fill-warn'}`}
                      ...
                      <div className="dp-metrics">
                        <div className="dp-m-item"><div className="dp-m-num">¥{formatMoney(pplHealth.currentPPL)}</div><div className="dp-m-desc">当前PPL</div></div>
                        <div className="dp-m-divider" />
                        <div className="dp-m-item"><div className="dp-m-num">¥{formatMoney(pplHealth.kpiTarget)}</div><div className="dp-m-desc">KPI目标</div></div>
                        <div className="dp-m-divider" />
                        <div className="dp-m-item"><div className={`dp-m-num ${pplHealth.kpiProbability >= 70 ? 'dp-m-ok' : 'dp-m-bad'}`}>{pplHealth.kpiProbability}%</div><div className="dp-m-desc">达成概率</div></div>
                        <div className="dp-m-divider" />
                        <div className="dp-m-item"><div className={`dp-m-num ${pplHealth.healthRatio >= 3 ? 'dp-m-ok' : 'dp-m-bad'}`}>{pplHealth.healthRatio.toFixed(1)}x</div><div className="dp-m-desc">PPL/KPI</div></div>
                      </div>
        ...
        case 'agent':
          if (logsAgentId) return <AutomationLogs agentId={logsAgentId} onBack={() => setLogsAgentId(null)} />;
          if (chatAgentId) return <AgentChat agentId={chatAgentId} onBack={() => setChatAgentId(null)} onNavigate={(key) => { setChatAgentId(null); setActiveMenu(key); }} />;
          return <AgentHub onChatOpen={(id) => setChatAgentId(id)} onLogsOpen={(id) => setLogsAgentId(id)} onCreateOpen={(tab) => { setAgentTab(tab); setShowCreateModal(true); }} />;
        default:
    ...
    if (isOps) {
      switch (activeMenu) {
        case 'ops-dashboard':
          return <OpsDashboard user={currentUser} />;
        case 'ops-report':
          return (
            <div>
              <div className="tm-tabs" style={{ marginBottom: 20 }}>
        ...
        case 'ops-project':
          return <OpsProject user={currentUser} />;
        case 'ops-alert':
          return <OpsAlert user={currentUser} />;
        case 'agent':
          if (logsAgentId) return <AutomationLogs agentId={logsAgentId} onBack={() => setLogsAgentId(null)} />;
          if (chatAgentId) return <AgentChat agentId={chatAgentId} onBack={() => setChatAgentId(null)} onNavigate={(key) => { setChatAgentId(null); setActiveMenu(key); }} />;
          return <AgentHub onChatOpen={(id) => setChatAgentId(id)} onLogsOpen={(id) => setLogsAgentId(id)} onCreateOpen={(tab) => { setAgentTab(tab); setShowCreateModal(true); }} />;
        default:
          setActiveMenu('ops-dashboard');
          return null;
      }
    }
    ...
    {alertDetail && (
      <div className="modal-overlay" onClick={() => setAlertDetail(null)}>
        <div className="modal-content da-detail-modal" onClick={e => e.stopPropagation()}>
          <div className="modal-header"><h3>告警详情</h3><button className="modal-close" onClick={() => setAlertDetail(null)}>&times;</button></div>
          <div className="da-detail-body">
            <div className="da-detail-row"><span className="da-detail-label">告警类型</span><span className="da-type-chip" style={{color: {red:'#f5222d',orange:'#fa8c16',yellow:'#d4b106',purple:'#722ed1'}[alertDetail.severity], background: {red:'#fff1f0',orange:'#fff7e6',yellow:'#fcffe6',purple:'#f9f0ff'}[alertDetail.severity]}}>{alertDetail.typeName}</span></div>
            <div className="da-detail-row"><span className="da-detail-label">告警对象</span><span className="da-detail-value">{alertDetail.object}</span></div>
            <div className="da-detail-row"><span className="da-detail-label">告警描述</span><span className="da-detail-value">{alertDetail.description}</span></div>
            <div className="da-detail-row"><span className="da-detail-label">告警时间</span><span className="da-detail-value">{alertDetail.time}</span></div>
            {isManager && alertDetail.salesName && <div className="da-detail-row"><span className="da-detail-label">所属销售</span><span className="da-detail-value">{alertDetail.salesName}</span></div>}
          </div>
          <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setAlertDetail(null)}>关闭</button></div>
        </div>
      </div>
    )}
    <AgentCreateModal visible={showCreateModal} defaultTab={agentTab} onClose={() => setShowCreateModal(false)} onCreated={() => setShowCreateModal(false)} />
    </>
  );
}

// Customer grade overview for manager dashboard
